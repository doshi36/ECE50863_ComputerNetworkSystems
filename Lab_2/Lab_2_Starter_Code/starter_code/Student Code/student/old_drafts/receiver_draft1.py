		...
		while True:
			try:
				...
				if packet_id == self.expected_packet:
					print(f"Receiver: Received packet {packet_id} from sender")
					if packet_data == b'END OF FILE!':
				...
				print(f"Receiver: Did not receive packet {self.expected_packet} from sender")
				self.recv_monitor.send(
										self.sender_id, 
						   				format_packet(self.receiver_id, self.sender_id, f"ACK {self.expected_packet-1}".encode('ascii'))
							   		  )
				continue
